...
- C++17コンパイラとOpenCV C++ライブラリ（動き補償ヘルパー用）
...
```sh
uv run python spc.py encode 20260519/D8A_2000.NEF 20260519/D8A_2001.NEF -o d8a_2001.spcraw
uv run python spc.py verify 20260519/D8A_2000.NEF 20260519/D8A_2001.NEF d8a_2001.spcraw
uv run python spc.py restore 20260519/D8A_2000.NEF d8a_2001.spcraw -o restored_D8A_2001.NEF
```
...
```sh
uv run python spc.py encode 20260519/D8A_2041.NEF 20260519/D8A_2042.NEF \
...
- `--diff-codec jxl` では、RGGBを4チャンネルに分離し、ECC affineでkeyframeをtargetへ合わせた予測画像との差分を16bit PAMへ格納し、JPEG XL Modular losslessで圧縮する
